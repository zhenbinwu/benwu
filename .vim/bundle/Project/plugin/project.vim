...
function! s:Project(filename) " <<<
    ...
    function! s:CreateEntriesFromDir(recursive)
        ...
        let home=''
        let filter='*'
        if (match(g:proj_flags, '\Cb') != -1) && has('browse') && has("gui_running")
            ...
            let dir = inputdialog('Enter the '.absolute.'Directory to Load: ', currentdir)
        endif
...
fun! s:Editlvimrc() "{{{
  ...
  if exists("g:loaded_syntastic_plugin") && g:loaded_syntastic_plugin == 1
    ...
    call add(out, 'let g:syntastic_cpp_compiler              = "' .
          \     (exists("g:syntastic_cpp_compiler")              ? g:syntastic_cpp_compiler : 'g++') . '"')
    ...
    call add(out, 'let g:syntastic_cpp_compiler_options      = "' .
          \     (exists("g:syntastic_cpp_compiler_options")      ? g:syntastic_cpp_compiler_options : ' -Wall') . '"')
