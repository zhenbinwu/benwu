...
function! s:insane_in_the_membrane() abort
  ...
  call append('$', ['   startify>', '', '   [i]  <empty buffer>'])
  let cnt = 0
  ...
  if exists('g:startify_bookmarks')
    ...
    for fname in g:startify_bookmarks
      ...
      call append('$', '   ['. index[cnt] .']'. repeat(' ', 3 - strlen(string(cnt))) . fname)
      execute 'nnoremap <buffer> '. index[cnt].' :edit '. startify#escape(fname) .'<cr>'
      let cnt += 1
    endfor
  endif
